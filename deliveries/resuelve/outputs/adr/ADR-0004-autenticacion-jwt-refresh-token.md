...
El backlog incluye historias de autenticación con requisitos de seguridad
específicos que fuerzan una decisión sobre la estrategia de gestión de sesión:

- **US-07** — Login en < 3 s; bloqueo temporal tras 5 intentos fallidos.
- **US-09** — Cierre de sesión desde cualquier pantalla (invalidación inmediata).
- **US-10** — La sesión expira por inactividad; el cliente es redirigido al login
  y regresa al lugar donde estaba.
...
|---|---|---|---|
| Access token | 15 minutos | Memoria de la APP (no persistido) | Adjunto a cada petición HTTP al API Gateway |
...
**Ciclo de vida:**
1. Login exitoso (US-07) → backend emite `access_token` (15 min) + `refresh_token` (7 días).
...
5. **Sesión expirada (US-10):** si el `refresh_token` también venció (7 días sin
   actividad), el intento de renovación falla → la APP redirige al login.
6. **Lockout (US-07):** el backend lleva el contador de intentos fallidos por
   `client_id`; no es responsabilidad del JWT. La APP solo muestra el mensaje de
   bloqueo que devuelve el servidor.
...
- **OAuth 2.0 completo con authorization server externo** — adecuado si Resuelve
  ...
  identidades federadas. Scope innecesario para MVP; la autenticación es con
  cédula + contraseña propia de Resuelve.
...
- El backend de Resuelve debe implementar el endpoint de revocación de refresh
  token. Debe incluirse en el contrato de API (OQ-01).
