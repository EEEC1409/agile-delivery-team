...
El backlog tiene 20 historias (60 pts) distribuidas en 5 épicas (Onboarding,
Autenticación, Dashboard, Incentivos, Notificaciones) que deben validarse con una
métrica de conversión ≥ 40 % en 7 días. El tiempo de entrega del MVP es la fuerza
dominante: no hay margen para mantener dos bases de código nativas independientes
...
- **Web App Progresiva (PWA)** — no soporta push notifications nativas fiables en
  iOS (limitaciones de Safari). Descartada porque US-18, US-19 y US-20 (EPIC-05)
  son funcionalidades MVP críticas.

## Consecuencias

**Ganamos:**
- Una sola base de código para iOS y Android.
- Ecosistema maduro para integración con FCM (push), JWT, formularios y navegación.
...
- Posibilidad de reusar lógica de negocio entre plataformas (validaciones,
  llamadas a API, manejo de tokens, interceptores de red — ADR-0005).
- La capa `NetworkLayer` (ADR-0005) se implementa una sola vez para ambas
  plataformas, eliminando duplicación en el manejo de errores de conectividad
  (US-11, R-36, R-41).
...
- Acceso a APIs de sistema operativo (biometría futura, R-13) requiere módulos
  nativos o librerías de terceros. No bloqueante para el MVP (biometría excluida
  en `mvp-canvas.md → Fuera de alcance`).
